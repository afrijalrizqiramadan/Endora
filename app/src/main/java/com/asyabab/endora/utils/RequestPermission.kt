...
class RequestPermission(
    ...
    private fun showRationaleIfNeed(): Boolean {
        ...
        val permanentDeniedPermissions =
            getPermission(unGrantedPermissions, Status.PERMANENT_DENIED)
        ...
        val temporaryDeniedPermissions =
            getPermission(unGrantedPermissions, Status.TEMPORARY_DENIED)
    ...
    private fun notifyComplete() {
        listener?.onComplete(
            getPermission(permissions, Status.GRANTED),
            getPermission(permissions, Status.UN_GRANTED)
        )
    }
